...
''' For applying sentiment: ##########################
1-3 stars : 0 - negative sentiment 
4-5 stars : 1 - positive sentiment 
'''
df_review['sentiment'] = df_review['stars'].apply(lambda rating : 1 if rating > 3 else 0) 

# Balanced dataset 
df_train_raw, df_test = train_test_split(df_review, test_size=500000, random_state=42, shuffle=True)

# Still resampling based on stars for equal distribution
